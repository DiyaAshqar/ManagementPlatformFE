  ...
  template: `
    <div class="card p-4">
      ...
      <div class="flex flex-col gap-4">
        <p>{{ 'wizard.step6.description' | translate }}</p>
        <div class="border-2 border-dashed border-surface-200 dark:border-surface-700 rounded bg-surface-50 dark:bg-surface-950 p-6 text-center">
          <p class="text-color-secondary">{{ 'wizard.step6.placeholder' | translate }}</p>
        </div>
        <p-button 
          [label]="'wizard.step6.validate' | translate" 
...
export class Step6Component {
  ...
  validateStep(): void {
    this.stepData.emit({ step6: 'placeholder data' });
  }
}
